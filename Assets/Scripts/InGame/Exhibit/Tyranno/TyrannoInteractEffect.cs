...
namespace InGame.Exhibit
{
    public class TyrannoInteractEffect : CharacterInteractEffectBase
    {
        ...
        private TyrannoDamageable _tyrannoDamageable;
        ...
        {
            Debug.Log($"{context} OnInteractStart,{target.name}");
            _cameraController = target.gameObject.GetComponent<CameraController>();
            _interactable = target;
            _rigidbody = target.gameObject.GetComponent<Rigidbody>();
            _cameraController.Init(true);
            _gameInput = new GameInput();
            _gameInput.Enable();
            _animator = target.gameObject.GetComponentInChildren<Animator>();
            _networkObject = target.gameObject.GetComponent<NetworkObject>();
            _networkRunner = _networkObject.Runner;
            _transform = target.transform;
            _tyrannoDamageable = target.gameObject.GetComponent<TyrannoDamageable>();
        ...
        {
            Debug.Log("OnInteractFixedNetworkUpdate");
        ...
        private bool CheckInteractEnd()
        {
            ...
            if(!_tyrannoDamageable.IsAlive) return true;
            return false;
        }

        private void GetOn(PlayerRef ownerPlayerRef)
        {
            Debug.Log("GetOn" + ownerPlayerRef);
            _ownerPlayerRef = ownerPlayerRef;
            _networkObject.AssignInputAuthority(ownerPlayerRef);
            Debug.Log(_networkObject.InputAuthority);
