...
{
    /// <summary>擬態アビリティの発動条件</summary>
    public class AbilityScanCondition : IAbilityExecuteCondition
    {
        public string TargetAbilityName => nameof(AbilityScan);
        ...
        public bool IsConditionMatch(in TriggerEventContext context)
        {
            ...
            return _playerMovement.IsGround && !_playerManager.IsStun && !IsGameEnded() &&
                   ...
                   context.CurrentButtons.GetPressed(context.PreviousButtons).IsSet(PlayerButtons.Ability1)
                   && isMimicking;
        }
