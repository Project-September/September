...
namespace September.Editor.PackageImporter
{
    ...
    internal static class AppsScriptClient
    {
            ...
            Action<string> onError)
        {
            ...
            string url = $"{webAppUrl}?action=download&id={UnityWebRequest.EscapeURL(file.id)}";
            ...
            using (var request = UnityWebRequest.Get(url))
            {
                ...
                if (HasError(request))
                {
                    onError?.Invoke($"ダウンロードに失敗しました ({file.name}): {request.responseCode} {request.error}");
                    yield break;
                }
                ...
                try
                {
                    response = JsonUtility.FromJson<DownloadResponse>(request.downloadHandler.text);
                }
                catch (Exception e)
                {
                    onError?.Invoke($"レスポンスの解析に失敗しました ({file.name}): {e.Message}");
                    yield break;
                }

                if (!string.IsNullOrEmpty(response?.error))
                {
                    onError?.Invoke($"Apps Script側エラー ({file.name}): {response.error}");
                    yield break;
                }

                if (string.IsNullOrEmpty(response?.base64))
                {
                    onError?.Invoke($"ファイル内容が空でした ({file.name})");
                    yield break;
                }
                ...
                {
                    onError?.Invoke($"ファイル保存に失敗しました ({file.name}): {e.Message}");
                    yield break;
                }
